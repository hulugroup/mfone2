...
//print_r(date('i:s:u') . '______9' . "\n");        
    ...
    protected function _processConfiguableProduct($product, &$importData)
    {
        ...
        if (!empty($newConfigurableAttributeIds)) {
            ...
            $newConfigurableAttributes = $product->getTypeInstance(true)->getConfigurableAttributesAsArray($product);
    ...
    protected function _processDownloadableProduct($product, &$importData)
    {
        ...
        foreach ($commadelimiteddata as $data) {
            ...
            if (isset($configBundleOptionsCodes[5])) {
                $downloadableitems['link'][$downloadableitemsoptionscount]['sample'] = array(
                    'file' => '[]',
                    'type' => 'url',
            ...
            if ($configBundleOptionsCodes[3] == "file") {
                ...
                if (!$file || !file_exists($file)) {
                    if(Mage::getModel("core/session")->getCountLostZipFile()>0){
                        $count = Mage::getModel("core/session")->getCountLostZipFile() + 1;
                    }else{
                        $count = 1;
                    }
                    Mage::getModel("core/session")->setCountLostZipFile($count);
                    $tmpLogName = preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '_', $importData["categories"]);
                    //Mage::log($importData["categories"]." : ".$files." - ".$count,null,$tmpLogName."_NOT_EXISTS_DOWNLOAD_FILES.log",true);
                    Mage::log($importData["sku"]." : ".$files." - ".$count,null,"NOT_EXISTS_DOWNLOAD_FILES.log",true);
            ...
            } elseif ($configBundleOptionsCodes[3] == "url") {
